Feature: POST Upgrade User Role
  ...
  Scenario: Post upgrade user role from 'user' to 'owner' with and wrong token valid body should failed then return 401 response code
    Given Post upgrade user role from 'user' to 'owner' with and wrong token valid body
    When Send request post upgrade user role from 'user' to 'owner'
